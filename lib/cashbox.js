...
module.exports = function(stores) {
	...
	Cache.prototype = {
		...
		set: function(/* key, value, tags, ttl, done */) {
			...
			if(args.length) {
				...
				if(typeof tags == 'object') {
					ttl = args.length && args.shift() || undefined;
				} else {
					ttl = tags;
					tags = undefined;
				}
			...
			this.store.set(key, value, tags, ttl, done);
		...
		mset: function(/* keysValues, tags, ttl, done */) {
			...
			this.store.mset(keysValues, tags, ttl, done);
		...
		getKeys: function(/* key, value, done */) {
			var args = slice.call(arguments),
				done = args.pop();
	...
	function loadMissingKeys(keys, values, load, done) {
		...
		load(missingKeys, function(err, values, tags) {
			...
			(values||[]).forEach(function(value, idx) {
				...
				tagMap[missingKeys[idx]] = tags[idx];
			});
