...
Memory.prototype = {
	...
	getKeys: function(hash, done) {
		var self = this;
		done(null, Object.keys(
			Array.prototype.concat.apply(
				[],
				Object.keys(hash).map(function(key) {
					var values = (hash[key] instanceof Array) ? hash[key] : [hash[key]];
					return Array.prototype.concat.apply(
						[],
						values.map(function(value) {
							return self.tags[[key, value].join(':')] || [];
						})
					);
				})
...
function setTags(key, tags, done) {
	...
	Object.keys(tags).forEach(function(name) {
		...
		values.forEach(function(value) {
			var set = [name, value].join(':');
			if(!self.tags[set]) self.tags[set] = [];
			self.tags[set].push(key);
		});
	});
}
